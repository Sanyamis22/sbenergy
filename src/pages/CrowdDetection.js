...
const CrowdDetection = props => {
  ...
  useEffect(async () => {
    setLoading(true);
    let response = await axios.get(`${API_URL}Fire/GetCrowd_Record`);
    if (response.status) {
  ...
  const getReport = async () => {
    setLoading(true)
    let response = await axios.get(`http://ayanapower-001-site1.itempurl.com/api/Fire/GetCrowd_Record_Report?filterType=All&startdate=${Moment(startdate).format('DD/MM/YYYY')}&enddate=${Moment(enddate).format('DD/MM/YYYY')}`)
    ...
    const html = `
    <center><h3 style="text-align:center">Crowd Detection Report</h3></center>
    ...
    const createpdf = async () => {
      setFilename('crowd-detection-'+ t_time)    
      if (await isPermitted()) {
        let options = {
          //Content to print
          html:html,
          //File Name
          fileName: 'crowd-detection-'+ t_time,
          //File directory
          directory: 'docs',
        };
