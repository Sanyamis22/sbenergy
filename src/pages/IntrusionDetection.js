...
const IntrusionDetection = props => {
  ...
  useEffect(async () => {
    setLoading(true);
    let response = await axios.get(`${API_URL}Fire/GetDetection_Data`);
    if (response.status) {
  ...
  const getReport = async () => {
    ...
    const html = `
    <center><h3 style="text-align:center">Intrusion Detection</h3></center>
    ...
    const createpdf = async () => {
      setFilename('intrusion_detection-'+ t_time)    
      if (await isPermitted()) {
        let options = {
          //Content to print
          html:html,
          //File Name
          fileName: 'intrusion_detection-'+ t_time,
          //File directory
          directory: 'docs',
        };
