...
const VegetationDetection = props => {
  ...
  useEffect(async () => {
    setLoading(true);
    let response = await axios.get(`${API_URL}Fire/GetOverGrass_Data`);
    if (response.status) {
  ...
  const getReport = async () => {
    setLoading(true)
    let response = await axios.get(`http://ayanapower-001-site1.itempurl.com/api/Fire/GetOverGrass_Data_Report?filterType=All&startdate=${Moment(startdate).format('DD/MM/YYYY')}&enddate=${Moment(enddate).format('DD/MM/YYYY')}`)
    ...
    const html = `
    <center><h3 style="text-align:center">Vegetation Detection Report</h3></center>
    ...
    const createpdf = async () => {
      setFilename('vegetation_detection-'+ t_time)    
      if (await isPermitted()) {
        let options = {
          //Content to print
          html:html,
          //File Name
          fileName: 'vegetation_detection-'+ t_time,
          //File directory
          directory: 'docs',
        };
