...
const FireAndSmoke = props => {
  ...
  useEffect(async () => {
    setLoading(true);
    let response = await axios.get(`${API_URL}Fire/GetFire_Record`);
    if (response.status) {
  ...
  const getReport = async () => {
    ...
    let response = await axios.get(
      `http://ayanapower-001-site1.itempurl.com/api/Fire/GetFire_Record_Report?filterType=All&startdate=${Moment(
        startdate,
      ).format('DD/MM/YYYY')}&enddate=${Moment(enddate).format('DD/MM/YYYY')}`,
    );

    const dynamicData = response.data.map(
      (row, i) =>
    ...
    const html = `
    <center><h3 style="text-align:center">Fire And Smoke Report</h3></center>
    <p style="margin-bottom:0px">${Moment(startdate).format(
      'DD-MM-YYYY',
    ...
    const createpdf = async () => {
      setFilename('fire_and_smoke-' + t_time);
      if (await isPermitted()) {
        let options = {
          //Content to print
          html: html,
          //File Name
          fileName: 'fire_and_smoke-' + t_time,
          //File directory
          directory: 'docs',
        };
      ...
      <Modal visible={visiblePopup}>
        ...
        <View
          ...
          <View style={styles.card_header}>
            <Text
              style={{color: '#446DFF', textAlign: 'center', marginTop: 15}}>
              Download Report
            </Text>
          </View>
