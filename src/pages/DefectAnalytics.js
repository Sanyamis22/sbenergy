...
const VegetationDetection = props => {
  ...
  useEffect(async () => {
    setLoading(true);
    let response = await axios.get(`${API_URL}Fire/GetGetmoduleDefect_Data`);
    if (response.status) {
  ...
  const getReport = async () => {
    setLoading(true)
    let response = await axios.get(`http://ayanapower-001-site1.itempurl.com/api/Fire/GetGetmoduleDefect_Data_Report?filterType=All&startdate=${Moment(startdate).format('DD/MM/YYYY')}&enddate=${Moment(enddate).format('DD/MM/YYYY')}`)
    ...
    const html = `
    <center><h3 style="text-align:center">Defect Analytics Report</h3></center>
    ...
    const createpdf = async () => {
      setFilename('Defect_analytics-'+ t_time)    
      if (await isPermitted()) {
        let options = {
          //Content to print
          html:html,
          //File Name
          fileName: 'Defect_analytics-'+ t_time,
          //File directory
          directory: 'docs',
        };
                      ...
                      <View style={styles.Shadowbox}>
                        ...
                        <View style={styles.description}>
                          ...
                          <View style={styles.desc_part}>
                            ...
                            <Text style={styles.label_data}>{item.RecDateTime}</Text>
                          </View>
